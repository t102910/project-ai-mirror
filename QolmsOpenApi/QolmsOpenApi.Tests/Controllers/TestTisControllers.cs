...
{
    [TestClass]
    public class TestTisControllers
    {
        ...
        [TestMethod]
        public void PostQrCodeRead_Success()
        {
            var args = new QoTisQrCodeReadApiArgs()
            {
                ApiType = QoApiTypeEnum.TisQrCodeRead .ToString(),
                ExecuteSystemType = QsApiSystemTypeEnum.QolmsOpenApi.ToString(),
                QRData = "X8jheT/kBI8GV3QiCG6/yzrvoCW/plhpBl5wc7H1O19vGKd0V0q+nX7rVgjI9xWKNvqEnqX0CFNmI0bfaq/A4IAWa/WIuFDaM+9YJ5ne+7YrbjnkCc1+09LrCjHQ8ui1EsD1F7YZ4tnz2YjA0WbV9w==",
                Version ="01",
            ...
            var result = controller.PostQrCodeRead(args);
            Assert.AreEqual(result.IsSuccess.TryToValueType(false), true);
            Assert.AreEqual(string.IsNullOrWhiteSpace(result.Birthday), false);
            Assert.AreEqual(string.IsNullOrWhiteSpace(result.CreatedDate ), false);
            Assert.AreEqual(string.IsNullOrWhiteSpace(result.FacilityKeyReference), false);
            Assert.AreEqual(string.IsNullOrWhiteSpace(result.FamilyKanaName ), false);
            Assert.AreEqual(string.IsNullOrWhiteSpace(result.FamilyName ), false);
            Assert.AreEqual(string.IsNullOrWhiteSpace(result.GivenKanaName ), false);
            Assert.AreEqual(string.IsNullOrWhiteSpace(result.GivenName), false);
            Assert.AreEqual(string.IsNullOrWhiteSpace(result.LinkUserId ), false);
            Assert.AreEqual(string.IsNullOrWhiteSpace(result.MedicalFacilityCode), false);
            Assert.AreEqual(string.IsNullOrWhiteSpace(result.SexType ), false);
        ...
        public void Test_Success()
        {
            ...
            string s=Microsoft.VisualBasic.Strings.StrConv("ﾄｸｼｭｳｶｲ ﾀﾛｳ", Microsoft.VisualBasic.VbStrConv.Wide, 0x411);
            Assert.AreNotEqual(nowstr.Length, 8);
        }
