...
namespace MGF.QOLMS.QolmsOpenApi.Controllers
{
    ...
    /// コントローラベースクラス
    /// </summary>
    public abstract class QoApiControllerBase : ApiController 
    {
        #region "Constant"
        ...
        /// </summary>
        /// <remarks></remarks>
        protected QoApiControllerBase() : base()
        {}
        ...
        /// </returns>
        /// <remarks></remarks>
        private bool CheckExecutor(QoApiArgsBase args, string encExecutor, ref QsApiSystemTypeEnum refExecuteSystemType, ref Guid refExecutor, ref string refExecutorName)
        {
        ...
        /// </returns>
        /// <remarks></remarks>
        private TResults ApiErrorResult<TResults>(QsApiSystemTypeEnum executeSystemType, Guid executor, Exception ex) where TResults : QoApiResultsBase, new()
        {
            var context = this.ControllerContext;
            // エラーログ
            Task.Run(() =>
            {
                QoAccessLog.WriteErrorLog(context, executeSystemType, executor, DateTime.Now, string.Empty, ex);
            });
            ...
            { 
                ...
                {  
                    Code = Convert.ToInt32(QoApiResultCodeTypeEnum.InternalServerError).ToString("d4"), 
        ...
        where TResults : QoApiResultsBase, new()
        {
            ...
            try
            {
                ...
                if (!this.CheckExecutor(args.DecryptMember<TArgs>(), encExecutor, ref executeSystemType, ref executor, ref executorName))
                    throw new ArgumentException(QoApiControllerBase.EXECUTOR_ERROR_MESSAGE);
                ...
                if (Request != null)
                {
                    ...
                    Task.Run(() =>
                    {
                        QoAccessLog.WriteAccessLog(QsApiSystemTypeEnum.QolmsOpenApi, executor, DateTime.Now, QoAccessLog.AccessTypeEnum.Api,
                            accessUri, "ExecuteWorkerMethod", userHostAddress, userHostName, userAgent);
                    });
                }
                // 処理メソッドを実行し、
            ...
            {
                QoAccessLog.WriteErrorLog(ex, executor);
                // エラー
        ...
        where TResults : QoApiResultsBase, new()
        {
            ...
            try
            {
                ...
                // 処理メソッドを実行し、
                // 必要に応じて戻り値クラスのプロパティ値を暗号化
                var result = (await method(args).ConfigureAwait(false)).EncryptMember<TResults>();
                // 成功時のコードは勝手に入るようにしとく
                if (result.Result == null && result.IsSuccess.TryToValueType(false))
                    result.Result = QoApiResult.Build(QoApiResultCodeTypeEnum.Success);
                return result;
