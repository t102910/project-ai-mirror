...
namespace MGF.QOLMS.QolmsOpenApi.Sql.Core
{
    ...
    /// </summary>
    /// <remarks></remarks>
    internal sealed class DbCheckupAfterSchoolFileWriterCore : QsDbWriterBase
    {
        ...
        /// </summary>
        /// <remarks></remarks>
        public DbCheckupAfterSchoolFileWriterCore() : base()
        {
        }
        ...
        /// </returns>
        /// <remarks></remarks>
        public int UpsertCheckupAfterSchoolFile(QH_CHECKUPAFTERSCHOOLFILE_DAT entity)
        {
            using (DbConnection connection = QsDbManager.CreateDbConnection<QH_CHECKUPAFTERSCHOOLFILE_DAT>())
            {
                ...
                query.Append("BEGIN");
                query.Append(" IF (SELECT COUNT(*) FROM QH_CHECKUPAFTERSCHOOLFILE_DAT WHERE LINKAGESYSTEMNO = @P1 AND LINKAGESYSTEMID = @P2 AND SERVICECODE = @P3 AND EFFECTIVEDATE = @P4 AND ORGANIZATIONID = @P5 AND SEQUENCE = @P6) <> 0");
                query.Append(" BEGIN");
                query.Append("  UPDATE QH_CHECKUPAFTERSCHOOLFILE_DAT SET");
                ...
                query.Append(" ELSE");
                query.Append(" BEGIN");
                query.Append("  INSERT INTO QH_CHECKUPAFTERSCHOOLFILE_DAT");
