using System;
using System.Collections.Generic;
using System.Data.Common;
using System.Text;
using System.Transactions;
using MGF.QOLMS.QolmsDbCoreV1;
using MGF.QOLMS.QolmsDbEntityV1;
using MGF.QOLMS.QolmsOpenApi.Sql.Core;

namespace MGF.QOLMS.QolmsOpenApi.Sql
{
    /// <summary>
    /// 
    /// </summary>
    internal sealed class LinkageWriter : QsDbWriterBase, IQsDbDistributedWriter<MGF_NULL_ENTITY, LinkageWriterArgs, LinkageWriterResults>
    {
        /// <summary>
        /// <see cref="LinkageWriter" /> クラスの新しいインスタンスを初期化します。
        /// </summary>
        /// <remarks></remarks>
        public LinkageWriter() : base()
        {
        }

        /// <summary>
        /// 分散トランザクションを使用してデータベーステーブルへ値を設定します。
        /// </summary>
        /// <param name="args">DB 引数クラス。</param>
        /// <returns>
        /// DB 戻り値クラス。
        /// </returns>
        /// <remarks></remarks>
        public LinkageWriterResults ExecuteByDistributed(LinkageWriterArgs args)
        {
            var result = new LinkageWriterResults { IsSuccess = false };            
            var actionDate = DateTime.Now;
            var actionKey = Guid.NewGuid();

            if(args.Entity.DataState == QsDbEntityStateTypeEnum.Added)
            {
                args.Entity.CREATEDDATE = actionDate;
            }
            args.Entity.UPDATEDDATE = actionDate;

            if (UpsertLinkageEntity(args.Entity))
            {
                // 処理件数
                result.Result = 1;
                // 操作日時
                result.ActionDate = actionDate;
                // 操作キー
                result.ActionKey = actionKey;
                // 成功
                result.IsSuccess = true;
            }                
           
            return result;
        }

        private bool UpsertLinkageEntity(QH_LINKAGE_DAT entity)
        {           
            QhLinkageEntityWriter writer = new QhLinkageEntityWriter();
            QhLinkageEntityWriterArgs writerArgs = new QhLinkageEntityWriterArgs() { Data = new List<QH_LINKAGE_DAT>() { entity } };
            QhLinkageEntityWriterResults writerResults = QsDbManager.WriteByCurrent(writer, writerArgs);


            if (writerResults.IsSuccess && writerResults.Result == 1)
            {
                return true;
            }

            return false;
        }
    }
}