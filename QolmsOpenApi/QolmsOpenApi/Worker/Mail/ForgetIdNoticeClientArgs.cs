...
namespace MGF.QOLMS.QolmsOpenApi.Worker.Mail
{
    ...
    /// </summary>
    /// <remarks></remarks>
    internal sealed class ForgetIdNoticeClientArgs : QoNoticeClientArgsBase
    {
        ...
        /// </summary>
        /// <remarks></remarks>
        private ForgetIdNoticeClientArgs() : base()
        {
        }

        /// <summary>
        /// 値を指定して、
        /// <see cref="ForgetIdNoticeClientArgs" /> クラスの新しいインスタンスを初期化します。
        /// </summary>
        /// <param name="settingsName">NoticeApi用の設定名。</param>
        /// <param name="mailAddress">送信先メールアドレス（受信者）の設定名。</param>
        /// <param name="forgetId">お問い合わせID。</param>
        ...
        /// <remarks></remarks>
        public ForgetIdNoticeClientArgs(string settingsName, string subjectName, string mailAddress, string forgetId, string bodyTemplatePath, string footerTemplatePath) : base(settingsName, subjectName, new List<string>() { mailAddress }, bodyTemplatePath, footerTemplatePath)
        {
            ...
            if (string.IsNullOrWhiteSpace(forgetId))
                throw new ArgumentNullException("forgetId", "forgetId が Null 参照もしくは空白です。");

            // 本文内のパラメータを置換
            this.Body = string.Format(this.Body, forgetId);

        }

        /// <summary>
        /// <see cref="ForgetIdNoticeClientArgs" /> クラスのインスタンスを初期化します。
        ...
        {
            // 基本クラスのプロパティの初期化
            this.SetSettingsNameFromAppSettings("MailSettingsNameForgetId");
            this.SetSubjectFromAppSettings("MailSubjectForgetId");
            try
            {
                this.SetBodyFromTemplate(HttpContext.Current.Server.MapPath("~/App_Data/MailBodyForgetId.txt"), HttpContext.Current.Server.MapPath("~/App_Data/MailFooter.txt"));
            }
            catch (Exception)
            {
