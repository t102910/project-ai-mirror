...
<Serializable()>
Public NotInheritable Class PortalUserInfomationEditInputModel
...
#Region "Constant"
    ...
    ''' <summary>
    ''' NOとして使用可能な文字を表す正規表現パターンを表します。
    ''' </summary>
    ''' <remarks></remarks>
    Private Const NO_PATTERN As String = "^[0-9]*$"
...
#Region "Public Property"
    ...
    ''' <summary>
    ''' 都道府県の沖縄県かどうかを取得または設定します。
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    <DisplayName("都道府県")>
    Public Property Prefectures As Byte = Byte.MinValue
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Property CityItemN As New List(Of CityItem)()
#End Region

#Region "Constructor"

    ''' <summary>
    ''' <see cref="PortalUserInfomationEditInputModel" /> クラスの新しいインスタンスを初期化します。
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub New()
        MyBase.New()
...
#Region "Public Method"
    ...
    ' ''' <remarks></remarks>
    'Public Sub UpdateByInput(inputModel As PortalUserInfomationEditModel) Implements IQyModelUpdater(Of PortalUserInfomationEditModel).UpdateByInput
...
#Region "IValidatableObject Support"
    ...
    Public Function Validate(validationContext As ValidationContext) As IEnumerable(Of ValidationResult) Implements IValidatableObject.Validate
        ...
        '半角英数に直す
        Dim rxNo As New Regex(NO_PATTERN, RegexOptions.Compiled)
        ...
        ' 居住
        ' 未指定:0 ～ 沖縄:47
        If Me.Prefectures < 0 AndAlso Me.Prefectures > 47 Then

            result.Add(New ValidationResult(String.Format(REQUIRED_ERRORMESSAGE), {"model.Prefectures"}))
        End If

        ' 市町村
        If Not String.IsNullOrWhiteSpace(Me.CityNo) Then
            If Not rxNo.IsMatch(Me.CityNo) Then result.Add(New ValidationResult(String.Format("市町村は選択肢から選択してください。"), {"model.PhoneNo"}))

        End If

        If Not String.IsNullOrWhiteSpace(Me.PhoneNo) Then
            ' 電話番号
            If Me.PhoneNo.Length < LENGTH_11 OrElse Me.PhoneNo.Length > LENGTH_12 Then result.Add(New ValidationResult("電話番号は11文字か12文字で入力してください。", {"model.PhoneNo"}))
            If Not rxNo.IsMatch(Me.PhoneNo) Then result.Add(New ValidationResult(String.Format(REGULAREXPRESSION_ERRORMESSAGE, "電話番号", "半角数字"), {"model.PhoneNo"}))
