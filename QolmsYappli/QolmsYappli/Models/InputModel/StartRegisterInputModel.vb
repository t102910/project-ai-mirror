...
Public NotInheritable Class StartRegisterInputModel
    Implements _
        IQyModelUpdater(Of StartRegisterInputModel),
        IValidatableObject
...
#Region "Public Property"
    ...
    Public ReadOnly Property SessionId As String
        ...
        Get
            Return StartRegisterInputModel.SESSION_ID
        End Get
    ...
    Public ReadOnly Property ApiAuthorizeKey As Guid
        ...
        Get
            Return StartRegisterInputModel.API_AUTHORIZE_KEY
        End Get
    ...
    <StringLength(StartRegisterInputModel.LENGTH_25, ErrorMessage:="{0}は{1}文字以下で入力してください。")>
    ...
    '<DisplayName("ユーザーID")>
    '<Required(AllowEmptyStrings:=False, ErrorMessage:="{0}は必須項目です。")>
    '<StringLength(StartRegisterInputModel.LENGTH_256, ErrorMessage:="{0}は{1}文字以下で入力してください。")>
    '<RegularExpression(StartRegisterInputModel.PASSWORD_PATTERN, ErrorMessage:="{0}に使用できない文字が含まれています。")>
    ...
    '<DisplayName("パスワード")>
    '<Required(AllowEmptyStrings:=False, ErrorMessage:="{0}は必須項目です。")>
    '<StringLength(StartRegisterInputModel.LENGTH_32, MinimumLength:=StartRegisterInputModel.LENGTH_8, ErrorMessage:="{0}は{2}文字以上、{1}文字以下で入力してください。")>
    '<RegularExpression(StartRegisterInputModel.PASSWORD_PATTERN, ErrorMessage:="{0}に使用できない文字が含まれています。")>
...
#Region "Constructor"
    ...
    ''' <summary>
    ''' <see cref="StartRegisterInputModel" /> クラスの新しいインスタンスを初期化します。
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub New()
        MyBase.New()
...
#Region "Public Method"
    ...
    ''' <remarks></remarks>
    Public Sub UpdateByInput(inputModel As StartRegisterInputModel) Implements IQyModelUpdater(Of StartRegisterInputModel).UpdateByInput
...
#Region "IValidatableObject Support"
    ...
    Public Function Validate(validationContext As ValidationContext) As IEnumerable(Of ValidationResult) Implements IValidatableObject.Validate
        ...
        '    result.Add(New ValidationResult("パスワードと、パスワード（確認）が異なります。", {"Password2"}))
        'End If

        '' メールアドレスを検証
        'If Not String.IsNullOrWhiteSpace(Me.MailAddress) Then
        '    Dim eM As New EmailAddressAttribute()
        '    If Not eM.IsValid(Me.MailAddress) Then
        '        result.Add(New ValidationResult("メールアドレスの形式が不正です。", {"Mail"}))
        '    End If
        'End If
        If Not Regex.IsMatch(Me.MailAddress, MAILADDRESS_PATTERN, RegexOptions.IgnoreCase) Then
            result.Add(New ValidationResult("メールアドレスの形式が不正です。", {"Mail"}))
        End If
