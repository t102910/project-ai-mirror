...
''' <summary>
''' 連携システムに関する共通機能を提供します。
''' このクラスは継承できません。
''' </summary>
Friend NotInheritable Class LinkageWorker
#Region "Constant"
...
#Region "Private Method"
    ...
    ''' <returns></returns>
    Private Shared Function ExecuteLinkageReadApi(mainModel As QolmsYappliModel, linkageSystemNo As Integer) As QjCommonLinkageReadApiResults

        Dim apiArgs As New QjCommonLinkageReadApiArgs(
        ...
        Dim apiResults As QjCommonLinkageReadApiResults = QsApiManager.ExecuteQolmsJotoApi(Of QjCommonLinkageReadApiResults)(
            apiArgs,
            mainModel.SessionId,
...
#Region "Private Method"
    ...
    ''' <summary>
    ''' 連携システム番号 を指定して、連携情報 を取得します。
    ''' </summary>
    ''' <param name="mainModel">メインモデル</param>
    ''' <param name="linkageSystemNo">連携システム番号</param>
    ''' <returns></returns>
    Public Shared Function GetLinkageItem(mainModel As QolmsYappliModel, linkageSystemNo As Integer) As LinkageItem

        Dim apiResult As QjCommonLinkageReadApiResults = LinkageWorker.ExecuteLinkageReadApi(mainModel, linkageSystemNo)
