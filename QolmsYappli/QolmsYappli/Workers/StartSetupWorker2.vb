...
Friend NotInheritable Class StartSetupWorker2
...
#Region "Private Method"
    ...
    Private Shared Function ExecuteStartSetUpWriteApi(mainModel As QolmsYappliModel, inputModel As StartSetupInputModel2, recordDate As Date) As QhYappliStartSetUpWriteApiResults

        Dim now As Date = Date.Now

        ' 期限日を変換
        Dim setTargetDate As String = String.Empty
        Dim tmpDate As Date = Date.MinValue
        If String.IsNullOrWhiteSpace(inputModel.TargetDate) = False AndAlso Date.TryParse(inputModel.TargetDate, tmpDate) Then
            setTargetDate = tmpDate.ToApiDateString()
        End If
...
#Region "Public Method"
    ...
    <Obsolete()>
    Public Shared Function CreateInputModel(mainModel As QolmsYappliModel) As StartSetupInputModel2

        ' TODO:
        Return New StartSetupInputModel2(mainModel)

    End Function

    <Obsolete()>
    Public Shared Function Edit(mainModel As QolmsYappliModel, inputModel As StartSetupInputModel2) As Boolean
        ...
        ' API を実行
        With StartSetupWorker2.ExecuteStartSetUpWriteApi(mainModel, inputModel, pointTargetDate)
