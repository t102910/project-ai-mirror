...
''' </summary>
''' <remarks></remarks>
Friend NotInheritable Class TargetValueWorker
...
#Region "Public Method"
    ...
    ''' <param name="targetValueN"></param>
    ''' <remarks></remarks>
    Public Shared Sub SetTargetValue(mainModel As QolmsYappliModel, targetValueN As List(Of QhApiTargetValueItem))

        ' 初期化
        Dim values As New Dictionary(Of Tuple(Of QyVitalTypeEnum, QyStandardValueTypeEnum), QhApiTargetValueItem)()

        TargetValueWorker.valueKeys.ToList().ForEach(
            Sub(i)
                values.Add(
                    New Tuple(Of QyVitalTypeEnum, QyStandardValueTypeEnum)(i.Item1, i.Item2),
                    TargetValueWorker.CreateTargetValue(
                        i.Item1,
                        i.Item2,
                        Decimal.MinusOne,
                        Decimal.MinusOne
        ...
        If targetValueN IsNot Nothing AndAlso targetValueN.Any() Then
            targetValueN.ForEach(
                Sub(i)
                    ...
                    If values.ContainsKey(key) Then TargetValueWorker.CopyTargetValue(i, values(key))
                End Sub
            )
        End If

        ' 目標値値を設定
        mainModel.AuthorAccount.TargetValues = values
    ...
    Public Shared Sub GetStandardValue(mainModel As QolmsYappliModel, vitalType As QyVitalTypeEnum, valueType As QyStandardValueTypeEnum, ByRef refLower As Decimal, ByRef refUpper As Decimal)
        ...
        StandardValueWorker.GetStandardValue(mainModel, vitalType, valueType, refLower, refUpper)

    End Sub

    ''' <summary>
    ''' バイタル情報の種別および目標値の種別を指定して、
    ''' バイタル目標値を取得します。
    ''' </summary>
    ''' <param name="mainModel">メイン モデル。</param>
    ''' <param name="vitalType">バイタル情報の種別。</param>
    ''' <param name="valueType">目標値の種別。</param>
    ''' <param name="refLower">目標下限値が格納される変数。</param>
    ''' <param name="refUpper">目標上限値が格納される変数。</param>
    ''' <remarks></remarks>
    Public Shared Sub GetTargetValue(mainModel As QolmsYappliModel, vitalType As QyVitalTypeEnum, valueType As QyStandardValueTypeEnum, ByRef refLower As Decimal, ByRef refUpper As Decimal)

        Dim value As QhApiTargetValueItem = TargetValueWorker.CreateTargetValue(QyVitalTypeEnum.None, QyStandardValueTypeEnum.None, Decimal.MinusOne, Decimal.MinusOne)
        Dim key As New Tuple(Of QyVitalTypeEnum, QyStandardValueTypeEnum)(vitalType, valueType)

        If mainModel.AuthorAccount.TargetValues.ContainsKey(key) Then TargetValueWorker.CopyTargetValue(mainModel.AuthorAccount.TargetValues(key), value)

        refLower = value.Lower.TryToValueType(Decimal.MinusOne)
        refUpper = value.Upper.TryToValueType(Decimal.MinusOne)

        ' 目標値が無ければ、標準値を返却する
        If refLower <= Decimal.Zero OrElse refUpper <= Decimal.Zero Then TargetValueWorker.GetStandardValue(mainModel, vitalType, valueType, refLower, refUpper)
