//  名前空間オブジェクト
mgf.portal.tanitaconnection = mgf.portal.tanitaconnection || {};

mgf.portal.tanitaconnection = (function () {
    ...
    function deviceRegister(name,checked,alkooCancelFlag){
        ...
        $.ajax({
            type: "POST",
            url: "../Portal/TanitaConnectionResult?ActionSource=Update",
            data: {
    ...
    $("main").on("click", "#connection-data input", function () {
        ...
        if (linked) {
            ...
            if (name == 'Pedometer' && alkoo && checked) {
                ...
                $("#alkoo-disconnect-modal").modal("show");
            } else {
                //console.log(name)
    ...
    $("main").on("click", "#alkoo-disconnect-modal button", function () {
        // 確認 ダイアログ を非表示
        $("#alkoo-disconnect-modal").modal("hide");
    ...
    function connect(alkooCancelFlag) {
        ...
        $.ajax({
            type: "POST",
            url: "../Portal/TanitaConnectionResult?ActionSource=Register",
            data: {
                __RequestVerificationToken: $("input[name='__RequestVerificationToken']").val(),
                ID: id,
                Password: password,
            ...
            try {
                if (jqXHR.status == 200 && jqXHR.getResponseHeader("Content-Type").indexOf("application/json;") >= 0 && $.isTrueString($.parseJSON(data)["IsSuccess"])) {
                    ...
                    //$("#finish-modal .modal-body").text($.parseJSON(data)["Message"]);
                    //$("#finish-modal").modal("show");
                    location.href = "../Portal/TanitaConnection"
    ...
    $("main").on("click", "#cancel", function () {
        ...
        //$.ajax({
        //    type: "POST",
        //    url: "../Portal/TanitaConnectionResult?ActionSource=Cancel",
        //    data: {
        //        __RequestVerificationToken: $("input[name='__RequestVerificationToken']").val()
        //    },
        //    async: true,
